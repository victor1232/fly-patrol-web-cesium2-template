...
/*
  多条件搜索函数
*/
export const vlookup = (dictionary: any, list: any[], callback: Function) => {
  const filteredList = list.reduce((acc, item) => {
    let shouldInclude = true;
    for (let key of Object.keys(dictionary)) {
      if (dictionary[key].value === "") continue;
      if (dictionary[key].type === "string") {
        const itemValue = item[key] ? item[key].replace(/ +/g, '') : '';
        const dictValue = dictionary[key].value.replace(/ +/g, '');
        if (!itemValue.includes(dictValue)) {
          shouldInclude = false;
          break;
        }
      } else if (dictionary[key].type === "date") {
        const itemValue = item[key];
        const dictValue = dictionary[key].value;
        if (itemValue !== dictValue) {
          shouldInclude = false;
          break;
        }
      }
